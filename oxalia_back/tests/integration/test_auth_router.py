...
@pytest.mark.asyncio
async def test_link_telegram_and_conflict(client):
    email = f"doc_{uuid.uuid4().hex}@test.com"
    password = "StrongPassw0rd!"
    await client.post(
        "/auth/register",
    ...
    resp = await client.patch(
        "/auth/me/telegram", json={"telegram_user_id": "555666777"}, headers=headers
    )
    assert resp.status_code == 200
    assert resp.json()["telegram_user_id"] == "555666777"
    ...
    conflict = await client.patch(
        "/auth/me/telegram",
        json={"telegram_user_id": "555666777"},
